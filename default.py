# -*- coding: utf-8 -*-
#------------------------------------------------------------
# PalcoTV - Kodi Addon by Juarrox (junalerio@gmail.com)
# Version 0.3.1 (22.04.2015)
#------------------------------------------------------------
...
home = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.javier.iptv/', ''))
tools = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.javier.iptv/resources/tools', ''))
addons = xbmc.translatePath(os.path.join('special://home/addons/', ''))
art = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.javier.iptv/art', ''))
cbx_pages = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.javier.iptv/art/cbx', ''))
tmp = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.javier.iptv/tmp', ''))
...
def main_list(params):
    ...
    try:
        ...
        mastermenu = 'https://dl.dropboxusercontent.com/u/8036850/PalcoTV/PalcoTV-0.3.1.xml'
        data = plugintools.read(mastermenu)
...
def multilink(params):
    ...
    #ToDo: Añadir control para evitar error si el EPG se muestra en título
    if title.find("  ") >= 0:
        title = title.split("  ")
        title = title[0].strip()
    ...
    encuentra = '#EXTINF:-1,' + title.replace(" [COLOR lightyellow][Multilink][/COLOR]","")
            ...
            if data.startswith(encuentra) == True or data.startswith('#EXTINF:-1,@' + title.replace(" [COLOR lightyellow][Multilink][/COLOR]","")) == True:
                ...
                if data == "#multilink":
                    ...
                    while linea_url != "#multilink" :
                        ...
                        title_fixed = multiparse_title(title_option, url_option)
...
def launch_magnet(url):    
    ...
    if addon_magnet == "Stream":  # Stream (por defecto)
        url = 'plugin://plugin.video.stream/play/'+url
    elif addon_magnet == "Pulsar":  # Pulsar
        url = 'plugin://plugin.video.pulsar/play?uri=' + url
    elif addon_magnet == "KMediaTorrent":  # Kmediatorrent
...
def cbx_reader(params):
    ...
    if url.find("mediafire") >= 0:  # ***************** MEDIAFIRE *****************
        ...
        src_cbx = tmp + filename+'.cbr'  # ExtensiÃ³n .rar para que no salte iteraciÃ³n posterior de url.endswith("url") == True en linea 6223
        ...
        progreso.create("PalcoTV", "Iniciando descarga de [I]Mediafire[/I] en [I][B]addons/tmp[/B][/I]\n")
        ...
        if os.path.isfile(src_cbx) is False:
            ...
            fh = open(tmp + file_compressed, "wb")  #open the file for writing
        ...
        if os.path.isfile(src_cbx) is False:
            ...
            if url.endswith("cbz") == True:
                progreso.create("PalcoTV", "Descargando archivo CBZ en [I][B]addons/tmp[/B][/I]")
            elif url.endswith("cbr") == True:
                progreso.create("PalcoTV", "Descargando archivo CBR en [I][B]addons/tmp[/B][/I]")
            elif url.find("copy.com") >= 0:
                if url.find("cbr") >= 0:
                    progreso.create("PalcoTV", "Descargando archivo CBR de Copy.com en [I][B]addons/tmp[/B][/I]")
                elif url.find("cbz") >= 0:
                    progreso.create("PalcoTV", "Descargando archivo CBZ de Copy.com en [I][B]addons/tmp[/B][/I]")
            # response = urllib2.urlopen(url)
            # body = response.read()
    ...
    #xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('PalcoTV', "DescompresiÃ³n fallida", 3 , art+'icon.png'))


    # os.remove(tmp + filename)  # Habilitar opciÃ³n en la configuraciÃ³n para borrar el archivo descargado
    # xbmc.executebuiltin("Container.SetViewMode(500)")
    # text = 'plugin://plugin.image.pdfreader/?mode=100&name='+title+'&url='+playlists+filename
    # runPlugin(text)
